# Project Settings
folder = "data" # Directory where input files, output files, models, and TensorFlow board logs will be saved.

# Script Settings
input_file = 'text1.txt'  # Path to the input text file
...
checkpoint_folder = f"training_checkpoints_v1"  # Directory for saving model checkpoints
...
rnn_units = 4  # Number of units in the RNN layer
model = "LSTM"  # Choose between GRU and LSTM (Enter 'GRU' or 'LSTM')
...
logging_file = 'output1.txt'  # File for logging the generated text output

# Saved model name (this will be saved in the data folder)
save_model = "model-test"

# Retrain Setting
retrain_epoch = 5 # Number of epochs for retraining the model.
